(function ($) {
  ...
  $.fn.bootstrapTreeTable = function (options, param) {
    ...
    if (options.toolbar) {
      var _tool_div = $(
          "<div class='fixed-table-toolbar' style='display:none;'></div>");
    ...
    target.getRootNodes = function (data) {
      ...
      $.each(data, function (index, item) {
        ...
        if (!item[options.parentCode] || (_root ? (item[options.parentCode]
            == options.rootCodeValue) : _defaultRootFlag)) {
          result.push(item);
        }
    ...
    target.renderRow = function (tr, item) {
      $.each(options.columns, function (index, column) {
        ...
        if (index == 0 && column.field == 'selectItem') {
          ...
          var td = $(
              '<td style="text-align:' + column.align + ';' + ((column.width)
              ? ('width:' + column.width) : '') + '"></td>');
          // 增加formatter渲染
          if (column.formatter) {
    ...
    target.load = function (parms) {
      ...
      $.each(options.columns, function (i, item) {
        ...
        if (i == 0 && item.field == 'selectItem') {
          ...
          th = $('<th style="text-align:' + item.valign + ';padding:10px;'
              + ((item.width) ? ('width:' + item.width) : '') + '"></th>');
        }
        th.text(item.title);
        thr.append(th);
      });
      ...
      $.ajax({
        ...
        success: function (data, textStatus, jqXHR) {
          ...
          // 初始化treegrid
          target.treegrid({
            treeColumn: options.expandColumn ? options.expandColumn
                : (hasSelectItem ? 1 : 0),//如果有radio或checkbox默认第二列层级显示，当前是在用户未设置的提前下
          ...
          target.find("tbody").find("tr").click(function () {
            if (hasSelectItem) {
              ...
              if (_ipt.attr("type") == "radio") {
                ...
                target.find("tbody").find("tr").removeClass(
                    "treegrid-selected");
