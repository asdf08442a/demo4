  ...
  $.fn.extend({
    upload: function (options, success, error) {
      ...
      var Uploader = {
        ...
        progressbar: $(
            '<div class="uploader-progressbar"><div><div></div></div></div>'),
        ...
        run: function () {
          ...
          uploader.on('uploadProgress', function (file, percentage) {
            var picker = document.getElementById(
                'rt_' + file.source.ruid).parentNode;
            if (options.progress) {
            ...
            for (var i = 0; i < exts.length; i++) {
              if (file.type.indexOf(exts[i]) >= 0 || file.ext.indexOf(exts[i])
                  >= 0) {
                extsCount++;
              }
            }
        ...
        addProgressbar: function (percentage, picker) {
          ...
          me.progressbar.children('div').attr('style',
              'width: ' + percentage.toFixed(2) * 100 + '%');
          //};
        }
      ...
      var Crop = {
        ...
        cropTemplate: ['<div class="cropper-photobox">',
          '	<div class="photobox">',
          '		<div class="close photobox-close">X</div>',
          '		<h4 class="photobox-header-title">剪切你的图片</h4>',
          '		<div class="cropper-wraper">',
          '    		<div class="img-container"><img class="cropper-img" src="" alt="" /></div>',
          ' 		</div>', '		<div class="photobox-footer text-right">',
          '			<a class="button tiny upload-btn">提交</a>', '		</div>',
          '	</div>',
          '	<div class="photobox-overlay photobox-overlay-active"></div>',
          '</div>',].join(''),
        ...
        initialize: function (uploader) {
          ...
          me.$photobox.on('click', function (event) {
            ...
            if (target == document.getElementsByClassName('photobox-close')[0]
